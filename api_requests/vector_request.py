...
class VectorRequest(BaseRequest):
    ...
    def __init__(self, req: func.HttpRequest, command: str = None):
        ...
        params_defaults = {
            ...
            "containerName": {
                ...
                "default": DEFAULT_WORKSPACE_CONTAINER,
            },
            ...
            "layerName": {"var": "layer_name", "default": None},
        ...
        if self.overwrite and self.append:
            ...
            error_message = ("overwrite and append parameters cannot both be set to True")
        ...
        if command == "stage":
            if self.file_name:
                if isinstance(self.table_name,str):
                    ...
                    command = 'return_error'
                    error_message = f"tableName parameter missing for file <{self.file_name}>"
                    logger.critical(f"{error_message}")
    ...
    def _list_tables(self):
        ...
        if hasattr(self, "req_json") and self.req_json:
            ...
            schema_name = self.req_json.get("schemaName", HOSTING_SCHEMA_NAME)
        else:
            geo_only = True
    ...
    def _stage(self):
        ...
        try:
            table_exists = D.table_exists(table_name=self.table_name, schema_name=self.schema_name)
        except Exception as e:
            error_message = f"Error checking if table exists in database"
            logger.critical(f"{error_message} {e}")
        ...
        if table_exists:
            if self.if_exists == "fail":
                return self.return_error(
                    f"Table {self.schema_name}.{self.table_name} already exists in database. Please rename table or set overwrite parameter to True to overwrite the table or set append to True to append data to the existing table"
                )
        ...
        if D.gdb_is_registered_table(
            table_name=self.table_name, schema_name=self.schema_name
        ):
            logger.info(
                f"Table {self.schema_name}.{self.table_name} already registered with Enterprise Geodatabase"
            )
        else:
            logger.debug("Registering table with Enterprise Geodatabase")
            try:
                E = EnterpriseClient.from_vault()
                registered = E.register_table(
                    table_name=self.table_name, schema_name=self.schema_name
                )
                if registered:

                    out_message = f"Data inserted into table: {self.schema_name}.{self.table_name} and registered with Enterprise Geodatabase"
                else:
                    raise GeoprocessingError(
                        f"Unknown error registering table {self.schema_name}.{self.table_name} with Enterprise Geodatabase"
                    )
            except Exception as e:
                out_message = f"Data inserted into table: {self.schema_name}.{self.table_name} with ERROR: Could not register table with Enterprise Geodatabase: {e} - please check database for table registration"
